-- TABLE wmvventsettings, adapted from the script ventilation-durations.sql from the MIT-LCP.


DROP TABLE IF EXISTS wmvventsettings CASCADE;
CREATE TABLE wmvventsettings AS
WITH t0 AS
(
SELECT
...
AND value NOT IN
(
	-- Possible values for itemid = 720
	'Tamponade'
	,'Fluid Bolus'
	,'OR Received'
	,'Dysrhythmia'
...
DROP TABLE IF EXISTS wmvventdurations CASCADE;
CREATE TABLE wmvventdurations AS
WITH vd0 AS
(
SELECT
	icustay_id
...
UPDATE wmvventdurations SET Tracheostomy = 1 WHERE icustay_id IN (SELECT DISTINCT(icustay_id) FROM wmvventsettings WHERE Tracheostomy = 1);
...
UPDATE wmvventdurations vd SET Method = 'Oxygenation or NIV flag'
FROM wmvventsettings vs
...
UPDATE wmvventdurations vd SET Method = 'Extubation flag'
FROM wmvventsettings vs
WHERE vd.icustay_id = vs.icustay_id
AND vs.extubation = 1
AND vs.charttime BETWEEN vd.endtime - INTERVAL '3' HOUR AND vd.endtime + INTERVAL '24' HOUR
...
-- Extubation success defined as :
-- 1) Not intubated in the next 72 hours
-- 2) Alive 72 hours after extubation
-- 3) Still in the unit in the 3 following hours (case when a patient exits the unit but intubated
-- Respect this order

DROP MATERIALIZED VIEW IF EXISTS weaningsuccess CASCADE;
CREATE MATERIALIZED VIEW weaningsuccess AS

WITH w0 AS(
	SELECT rownb, wmv.icustay_id, ie.hadm_id, ie.subject_id, NumInvasiveVent, StartTime, EndTime, VentHours, outtime
	FROM wmvventdurations wmv
	LEFT JOIN icustays ie ON wmv.icustay_id = ie.icustay_id
	WHERE
	wmv.icustay_id NOT IN (SELECT DISTINCT(icustay_id) FROM wmvventdurations WHERE Tracheostomy = 1)
	-- It is difficult to set an extubation time for patients with a tracheostomy
	-- (changing from invasive to non invasive ventilation or oxygenation frequently during the ICU stay)
)
,w1 AS(
	SELECT rownb, w0.icustay_id, w0.hadm_id, w0.subject_id, NumInvasiveVent, StartTime, EndTime, VentHours, outtime, adm.deathtime, pat.dod
	FROM w0
	LEFT JOIN patients pat ON w0.subject_id = pat.subject_id
	LEFT JOIN admissions adm ON w0.hadm_id = adm.hadm_id
)
SELECT rownb, icustay_id, hadm_id, subject_id, NumInvasiveVent, StartTime, EndTime, VentHours, outtime, deathtime, dod
,CASE
	WHEN NumInvasiveVent > 0 THEN
	CASE
		WHEN LEAD(starttime, 1) OVER(PARTITION BY icustay_id ORDER BY NumInvasiveVent) < endtime + INTERVAL '72' HOUR THEN 'Reintubation'
		WHEN deathtime IS NOT NULL AND deathtime <= endtime + INTERVAL '6' HOUR THEN 'Death during MV'
		WHEN deathtime IS NOT NULL AND deathtime > endtime + INTERVAL '6' HOUR AND deathtime < endtime + INTERVAL '72' HOUR THEN 'Death'
		WHEN deathtime IS NULL AND dod IS NOT NULL AND endtime < dod + INTERVAL '72' HOUR THEN 'Death'
		WHEN outtime < endtime + INTERVAL '6' HOUR THEN 'Discharge'
		ELSE 'Success' END
	ELSE NULL
END AS WeaningSuccess
FROM w1
;

UPDATE wmvventdurations wmv
SET WeaningSuccess = ws.WeaningSuccess, hadm_id = ws.hadm_id, subject_id = ws.subject_id
FROM weaningsuccess ws
WHERE wmv.rownb = ws.rownb;
